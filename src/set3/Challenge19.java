...
public class Challenge19 {
	static String[] strings = { "SSBoYXZlIG1ldCB0aGVtIGF0IGNsb3NlIG9mIGRheQ==", "Q29taW5nIHdpdGggdml2aWQgZmFjZXM=",
	...
	public static void main(String[] args) throws IOException {
		...
		byte[][] ciphertexts = new byte[strings.length][];
		int c = 0;
		for (String str : strings) {
			ciphertexts[c] = AES.CTR(ToByte.base64Decode(str), key, 0);
			c++;
		}
		String[] res = crackManual(ciphertexts);
		for(String r : res) {
			System.out.println(r);
		}
		// SPOILER: http://www.online-literature.com/frost/779/

	}
	
	
	private static String[] crackManual(byte[][] ciphertexts) {
		System.out.println("For a correct guess of character, press y + <enter>. Otherwise, just hit enter");
		int maxLength = 0;
		...
		String[] plain = new String[ciphertexts.length];
		Arrays.fill(plain, "");
		int i = 0;
		while(i < maxLength) {
			int options = 0;
			...
			while(key < 256) {
				...
				for(int j = 0; j < ciphertexts.length; j++) {
					byte[] c = ciphertexts[j];
					if(c.length <= i) continue;
					if(!isValidChar((byte)(key ^ c[i]))) {
						ok = false;
						break;
					}
				}
				if(ok) {
					...
					for(int j = 0; j < ciphertexts.length; j++) {
						if(ciphertexts[j].length <= i) continue;
						byte newChar = (byte)(key ^ ciphertexts[j][i]);
						System.out.println(plain[j] + (char)newChar);
					}
					String res = scan.nextLine();
					if(res.equals("y")) {
						for(int j = 0; j < ciphertexts.length; j++) {
							if(ciphertexts[j].length <= i) continue;
							plain[j] += (char)(byte)(key ^ ciphertexts[j][i]) + "";
						}
						break;
					}
				}
	...
	private static boolean isValidChar(byte i) {
		if(i > 122) return false;
		if(i < 32) return false;
		if(i > 34 && i != 39 && i != 44 && i != 45 && i != 46 && i < 58) return false;
		if(i > 90 && i < 97) return false;
		if(i > 59 && i < 63) return false;
